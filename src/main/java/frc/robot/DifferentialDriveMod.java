 ...
 * the same response.
 */
public class DifferentialDriveMod extends RobotDriveBase implements Sendable, AutoCloseable {
   ...
   */
  public DifferentialDriveMod(SpeedController leftMotor, SpeedController rightMotor) {
  ...
  public void curvatureDrive(double xSpeed, double zRotation, boolean isQuickTurn) {
    if (!m_reported) {
      HAL.report(tResourceType.kResourceType_RobotDrive, tInstances.kRobotDrive2_DifferentialCurvature, 2);
      m_reported = true;
    }
   ...
   * Checks for drift.
   * 
   * @return Value proportional to the error from a starting angle.
   */
  public double driftCheck() {
    ...
    if (currentChange < 0.25 && currentChange > -0.25) {
      currentAngle = 0;
    }
    // if (currentAngle - lastAngle != 0) { System.out.println("current difference-
    // " + currentChange); }
    lastAngle = currentAngle;
    return (double) currentChange * 0.25;
  }
  ...
  public void tankDrive(double leftSpeed, double rightSpeed, boolean squareInputs) {
    if (!m_reported) {
      HAL.report(tResourceType.kResourceType_RobotDrive, tInstances.kRobotDrive2_DifferentialTank, 2);
      m_reported = true;
    }
